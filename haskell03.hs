...
addSuffix suffix lisSuffix = [x ++ suffix | x <- lisSuffix]
...
sumOdds :: [Int] -> Int
sumOdds mista = foldl1 (\x y -> x + y) [x | x <- mista, odd x]

--9)Resolva a questão anterior sem usar list comprehension, criando a função sumOdds' :: [Int] -> Int.
sumOdds' :: [Int] -> Int
sumOdds' mista' = foldl1 (\x y -> x + y) (filter odd mista')
...
selectExpr :: [Int] -> [Int]
selectExpr val = [x | x <- val, even x, x > 20, x < 50]
...
countShorts strg = length [x | x <- strg, length x < 5]
...
calcExpr lisF = filter (>10) [x ^ 2 / 2 | x <- lisF]
...
--8
dotProd :: [Int] -> [Int] -> Int
dotProd nums1 nums2 = foldl1 (\x y -> x + y) (zipWith (*) [x | x <- nums1] [x | x <- nums2])